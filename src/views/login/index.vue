<template>
  <div class="login-container">
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form" auto-complete="on" label-position="left">
      ...
      <!-- <div class="wjmm">
        <router-link tag="div" to='/password'  style="height:55px;" class="nav-link forget">
            <el-button  @click="handleHistory">忘记密码</el-button>
        </router-link>
      </div> -->
      
      <!-- <div style="font-size: 12px;color: #666666;"><i style="width: 20px;height: 20px;color: #fff;" class="el-icon-warning"></i>初始密码为身份证后六位。</div> -->
<!--      <div class="tips">-->
<!--        <span style="margin-right:20px;">idCardNo: admin</span>-->
...
export default {
  ...
  data() {
    ...
    return {
      ...
      loginRules: {
        // idCardNo: [{ required: true, trigger: 'blur', validator: validateidCardNo }],
  ...
  methods: {
    ...
    szm(){
      ...
      getCaptcha({ uuid: this.$data.loginForm.uuid }).then(response => {
        this.$data.imgUrl = 'http://hainantraining.qspfwadmin.com/login/getCaptcha?uuid=' + this.$data.loginForm.uuid
        resolve(data)
      }).catch(error => {
      })
    }
